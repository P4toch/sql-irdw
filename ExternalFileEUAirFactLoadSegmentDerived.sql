if object_id('dbo.ExternalFileEUAirFactLoadSegmentDerived') is null begin
    print 'Creating stored procedure ExternalFileEUAirFactLoadSegmentDerived (placeholder)'
    execute('create procedure dbo.ExternalFileEUAirFactLoadSegmentDerived as return 0')
end
go

print 'Altering stored procedure ExternalFileEUAirFactLoadSegmentDerived'
go

alter procedure dbo.ExternalFileEUAirFactLoadSegmentDerived
    @pExternalFileID int
...
Description:
     Updates ExternalFileEUAirFact with the derived attributes
...
(
 ...
 from dbo.ExternalFileEUAirFlightFact a 
  inner join dbo.ExternalFileEUAirFact b on
    a.ExternalFileID = b.ExternalFileID and a.RecordKey = b.RecordKey and
...
) as B on  a.ExternalFileID = b.ExternalFileID and a.RecordKey = b.RecordKey and 
...
if (@Error <> 0) begin
    select @ErrorCode   = @ERRUNEXPECTED,
           @MsgParm1    = cast(@Error as varchar(12)) + ' (update ExternalFileEUAirFact look up GeographyTypeID)'
