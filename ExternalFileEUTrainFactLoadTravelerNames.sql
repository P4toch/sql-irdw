if object_id('dbo.ExternalFileEUTrainFactLoadTravelerNames') is null begin
    print 'Creating stored procedure ExternalFileEUTrainFactLoadTravelerNames (placeholder)'
    execute('create procedure dbo.ExternalFileEUTrainFactLoadTravelerNames as return 0')
end
go

print 'Altering stored procedure ExternalFileEUTrainFactLoadTravelerNames'
go
...
alter procedure dbo.ExternalFileEUTrainFactLoadTravelerNames
    @pExternalFileID int
...
Description:
     Updates ExternalFileEUTrainFact with TravelerNameIDs
...
select distinct a.ExternalFileID, a.RecordKey, a.BookingTypeID, 
  ...
  from dbo.ExternalFileEUTrainFact a 
       inner join 
       dbo.ExternalFileEUTrainTicket b on a.ExternalFileID = b.ExternalFileID and a.RecordKey = b.RecordKey and a.BookingTypeID = b.BookingTypeID
       left outer join
...
select @TranStartedBool = @TRUE
    ...
    select TUID, CustomerSystemID, a.TitleTxt, FirstName, MiddleName, LastName, UpdateDate, LastUpdatedBy
      from
         (select distinct a.TUID, 2 as CustomerSystemID, a.TitleTxt, a.FirstName, a.MiddleName, a.LastName, @Current_Timestamp as UpdateDate, 'EFEUTrainFactLoadNames' as LastUpdatedBy
           from #NamesTraveler a 
                left join
    ...
    if (@Error <> 0) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = cast(@Error as varchar(12)) + ' (update ExternalFileEUTrainFact TravelerNameID)'
        raiserror (@ErrorCode, 16, 1, @ProcedureName, @MsgParm1)
        goto ErrorHandler
    end
    ...
    if (@Error <> 0) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = cast(@Error as varchar(12)) + ' (update ExternalFileEUTrainFact TravelerNameIDApproval)'
        raiserror (@ErrorCode, 16, 1, @ProcedureName, @MsgParm1)
        goto ErrorHandler
    end
...
begin tran
    exec ExternalFileEUTrainFactLoadTravelerNames @pExternalFileID
