if object_id('dbo.ExternalFileEUAirTicketFlightVldKey_') is null begin
    execute('create procedure dbo.ExternalFileEUAirTicketFlightVldKey_ as return 0')
end
go

print 'Altering stored procedure ExternalFileEUAirTicketFlightVldKey_'
go

alter procedure dbo.ExternalFileEUAirTicketFlightVldKey_ (
    @pExternalFileID int,
...
Description:
    Checks deletes potential PK violations in the EU air Flight 
...
-- Declare Cursor
---------------------------------------------------------------------
insert into @Flight (ExternalFileID, RecordKey, TripNbr, DuplicateCount)
select ExternalFileID,RecordKey,TripNbr, count(*)
  from dbo.ExternalFileEUAirTicketFlightStaging
 where ExternalFileID = @pExternalFileID
 group by ExternalFileID, RecordKey, TripNbr
having count(*) > 1

declare Flight_Cursor cursor local dynamic for
select a.ExternalFileID, a.RecordKey, a.TripNbr
  from dbo.ExternalFileEUAirTicketFlightStaging a
       inner join
       @Flight b on a.ExternalFileID = b.ExternalFileID and 
...
while( 1=1 ) begin
    ...
    fetch next from Flight_Cursor into @ExternalFileID, @RecordKey, @TripNbr
    select @Error = @@error
    if( @Error <> 0 ) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = isnull(convert(varchar,@Error),'NULL')
    ...
    if exists (select *
                ...
                where ExternalFileID = @ExternalFileID and
                      ...
                      DuplicateCount > 1)
        begin
            delete from dbo.ExternalFileEUAirTicketFlightStaging
...
ExitProc:
    ...
    */
    ...
    if cursor_status( 'local', 'Flight_Cursor' )
