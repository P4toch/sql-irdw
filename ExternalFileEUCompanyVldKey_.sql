if object_id('dbo.ExternalFileEUCompanyVldKey_') is null begin
    execute('create procedure dbo.ExternalFileEUCompanyVldKey_ as return 0')
end
go

print 'Altering stored procedure ExternalFileEUCompanyVldKey_'
go

alter procedure dbo.ExternalFileEUCompanyVldKey_ (
    @pExternalFileID int,
...
Description:
    Checks deletes potential PK violations in the EU company 
...
-- Declare Cursor
---------------------------------------------------------------------
insert into @Company (ExternalFileID, ComCode, DuplicateCount)
select ExternalFileID, Comcode, count(*)
  from dbo.ExternalFileEUCompanyStaging
 where ExternalFileID = @pExternalFileID
 group by ExternalFileID, Comcode
having count(*) > 1

declare Key_Cursor cursor local dynamic for
select a.ExternalFileID, a.Comcode
  from dbo.ExternalFileEUCompanyStaging a
       inner join
       @Company b on a.ExternalFileID = b.ExternalFileID and 
                     a.Comcode = b.Comcode 
for update
...
while( 1=1 ) begin
    ...
    fetch next from Key_Cursor into @ExternalFileID, @Comcode
    select @Error = @@error
    if( @Error <> 0 ) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = isnull(convert(varchar,@Error),'NULL')
    ...
    if exists (select *
                ...
                where ExternalFileID = @ExternalFileID and
                      ...
                      DuplicateCount > 1)
        begin
            delete from dbo.ExternalFileEUCompanyStaging
