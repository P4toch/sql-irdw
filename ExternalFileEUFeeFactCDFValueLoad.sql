if object_id('dbo.ExternalFileEUFeeFactCDFValueLoad') is null begin
    print 'Creating stored procedure ExternalFileEUFeeFactCDFValueLoad (placeholder)'
    execute('create procedure dbo.ExternalFileEUFeeFactCDFValueLoad as return 0')
end
go

print 'Altering stored procedure ExternalFileEUFeeFactCDFValueLoad'
go

alter procedure dbo.ExternalFileEUFeeFactCDFValueLoad 
    @pExternalFileID int
with recompile
as
...
Description:
    Inserts/Updates ExternalFileEUFeeFactCDFValue with new and modified
        data
...
insert into dbo.ExternalFileEUFeeFactCDFValue( ExternalFileID, RecordKey, BookingTypeID, CustomDataFieldID, CustomDataFieldValue ) 
    select z.ExternalFileID, z.RecordKey, z.BookingTypeID, z.CustomDataFieldID, z.CustomDataFieldValue
        from (
            ...
            join dbo.vExternalFileEUFeeCDFUnpivot b
...
if (@Error <> 0) begin
    select @ErrorCode   = @ERRUNEXPECTED,
           @MsgParm1    = cast(@Error as varchar(12)) + ' (insert ExternalFileEUFeeFactCDFValue)'
