if object_id('dbo.ExternalFileEUTrainStationVldKey_') is null begin
    execute('create procedure dbo.ExternalFileEUTrainStationVldKey_ as return 0')
end
go

print 'Altering stored procedure ExternalFileEUTrainStationVldKey_'
go

alter procedure dbo.ExternalFileEUTrainStationVldKey_ (
    @pExternalFileID int,
...
/*
*********************************************************************
Copyright (C) 2007-2012 Expedia, Inc. All rights reserved.

Description:
    Checks deletes potential PK violations in the EU TrainStation 
...
Change History:
    ...
    ----------  --------------- ------------------------------------
    2007-04-15  Barry Courtois Created.
...
declare     -- SP specific constants and variables
    ...
    @UpdateCount                    int,
    @ExternalFileID                 int, 
    @TrainStationCode               varchar(16),
    @DuplicateCount                 int

declare @TrainStation table
    (ExternalFileID int,
     TrainStationCode varchar(16),
     DuplicateCount int)
...
-- Declare Cursor
---------------------------------------------------------------------
insert into @TrainStation (ExternalFileID, TrainStationCode, DuplicateCount)
select ExternalFileID, TrainStationCode, count(*)
  from dbo.ExternalFileEUTrainStationStaging
 where ExternalFileID = @pExternalFileID
 group by ExternalFileID, TrainStationCode
having count(*) > 1

declare Key_Cursor cursor local dynamic for
select a.ExternalFileID, a.TrainStationCode
  from dbo.ExternalFileEUTrainStationStaging a
       inner join
       @TrainStation b on a.ExternalFileID = b.ExternalFileID and 
                          a.TrainStationCode = b.TrainStationCode 
for update
...
while( 1=1 ) begin
    ...
    fetch next from Key_Cursor into @ExternalFileID, @TrainStationCode
    select @Error = @@error
    if( @Error <> 0 ) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = isnull(convert(varchar,@Error),'NULL')
    ...
    if exists (select *
                ...
                where ExternalFileID = @ExternalFileID and
                      TrainStationCode = @TrainStationCode and
                      DuplicateCount > 1)
        begin
            delete from dbo.ExternalFileEUTrainStationStaging
