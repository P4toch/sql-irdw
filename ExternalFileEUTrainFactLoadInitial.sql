if object_id('dbo.ExternalFileEUTrainFactLoadInitial') is null begin
    print 'Creating stored procedure ExternalFileEUTrainFactLoadInitial (placeholder)'
    execute('create procedure dbo.ExternalFileEUTrainFactLoadInitial as return 0')
end
go

print 'Altering stored procedure ExternalFileEUTrainFactLoadInitial'
go

alter procedure dbo.ExternalFileEUTrainFactLoadInitial 
    @pExternalFileID int
with recompile
as
...
Description:
     Inserts index set of train fact records for a particular
     external file.
...
create table #ExternalFileEUTrainFact (
...
insert into #ExternalFileEUTrainFact (ExternalFileID, RecordKey, BookingTypeID)
select a.ExternalFileID, a.RecordKey, coalesce(a.BookingTypeID,0)
  from dbo.ExternalFileEUTrainTicket a
...
delete from #ExternalFileEUTrainFact where BookingTypeID not in (select BookingTypeID from BookingTypeDim)
...
select @TranStartedBool = @TRUE
    ...
    insert into dbo.ExternalFileEUTrainFact (ExternalFileID, RecordKey, BookingTypeID, UpdateDate, LastUpdatedBy)
    select a.ExternalFileID, a.RecordKey, a.BookingTypeID, @Current_TimeStamp, 'EFEUTrainFactLoadIntial'
      from #ExternalFileEUTrainFact a

    select @Error = @@Error
    if (@Error <> 0) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = cast(@Error as varchar(12)) + ' (insert to ExternalFileTrainFact)'
        raiserror (@ErrorCode, 16, 1, @ProcedureName, @MsgParm1)
        goto ErrorHandler
    end
