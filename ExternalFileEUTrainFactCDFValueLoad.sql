if object_id('dbo.ExternalFileEUTrainFactCDFValueLoad') is null begin
    print 'Creating stored procedure ExternalFileEUTrainFactCDFValueLoad (placeholder)'
    execute('create procedure dbo.ExternalFileEUTrainFactCDFValueLoad as return 0')
end
go

print 'Altering stored procedure ExternalFileEUTrainFactCDFValueLoad'
go

alter procedure dbo.ExternalFileEUTrainFactCDFValueLoad 
    @pExternalFileID int
with recompile
as
...
Description:
    Inserts/Updates ExternalFileEUTrainFactCDFValue with new and modified
        data
...
insert into dbo.ExternalFileEUTrainFactCDFValue( ExternalFileID, RecordKey, BookingTypeID, CustomDataFieldID, CustomDataFieldValue ) 
    select z.ExternalFileID, z.RecordKey, z.BookingTypeID, z.CustomDataFieldID, z.CustomDataFieldValue
        from (
            ...
            join dbo.vExternalFileEUTrainTicketCDFUnpivot b
...
if (@Error <> 0) begin
    select @ErrorCode   = @ERRUNEXPECTED,
           @MsgParm1    = cast(@Error as varchar(12)) + ' (insert ExternalFileEUTrainFactCDFValue)'
