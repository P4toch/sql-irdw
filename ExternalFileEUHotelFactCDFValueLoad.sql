if object_id('dbo.ExternalFileEUHotelFactCDFValueLoad') is null begin
    print 'Creating stored procedure ExternalFileEUHotelFactCDFValueLoad (placeholder)'
    execute('create procedure dbo.ExternalFileEUHotelFactCDFValueLoad as return 0')
end
go

print 'Altering stored procedure ExternalFileEUHotelFactCDFValueLoad'
go

alter procedure dbo.ExternalFileEUHotelFactCDFValueLoad 
    @pExternalFileID int
with recompile
as
...
Description:
    Inserts/Updates ExternalFileEUHotelFactCDFValue with new and modified
        data
...
insert into dbo.ExternalFileEUHotelFactCDFValue( ExternalFileID, RecordKey, BookingTypeID, CustomDataFieldID, CustomDataFieldValue ) 
    select z.ExternalFileID, z.RecordKey, z.BookingTypeID, z.CustomDataFieldID, z.CustomDataFieldValue
        from (
            ...
            join dbo.vExternalFileEUHotelCDFUnpivot b
...
if (@Error <> 0) begin
    select @ErrorCode   = @ERRUNEXPECTED,
           @MsgParm1    = cast(@Error as varchar(12)) + ' (insert ExternalFileEUHotelFactCDFValue)'
