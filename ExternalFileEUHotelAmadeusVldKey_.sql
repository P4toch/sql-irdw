if object_id('dbo.ExternalFileEUHotelAmadeusVldKey_') is null begin
    execute('create procedure dbo.ExternalFileEUHotelAmadeusVldKey_ as return 0')
end
go

print 'Altering stored procedure ExternalFileEUHotelAmadeusVldKey_'
go                               

alter procedure dbo.ExternalFileEUHotelAmadeusVldKey_ (
    @pExternalFileID int,
...
Description:
    Checks deletes potential PK violations in the EU Hotel Amadeus 
...
Change History:
    ...
    ----------  --------------- ------------------------------------
    2007-01-14   Barry Courtois  Created.
*********************************************************************
*/
set nocount on
...
-- Declare Cursor
---------------------------------------------------------------------
insert into @Stay (ExternalFileID, RecordKey, DuplicateCount)
select ExternalFileID,RecordKey, count(*)
  from dbo.ExternalFileEUHotelAmadeusStaging
 where ExternalFileID = @pExternalFileID
 group by ExternalFileID, RecordKey
having count(*) > 1

declare Stay_Cursor cursor local dynamic for
select a.ExternalFileID, a.RecordKey
  from dbo.ExternalFileEUHotelAmadeusStaging a
       inner join
       @Stay b on a.ExternalFileID = b.ExternalFileID and 
...
while( 1=1 ) begin
    ...
    fetch next from Stay_Cursor into @ExternalFileID, @RecordKey
    select @Error = @@error
    if( @Error <> 0 ) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = isnull(convert(varchar,@Error),'NULL')
    ...
    if exists (select *
                ...
                where ExternalFileID = @ExternalFileID and
                      ...
                      DuplicateCount > 1)
        begin
            delete from dbo.ExternalFileEUHotelAmadeusStaging
            where current of Stay_Cursor
...
ExitProc:
    ...
    */
    ...
    if cursor_status( 'local', 'Stay_Cursor' )
