if object_id('dbo.ExternalFileEUPercodeVldKey_') is null begin
    execute('create procedure dbo.ExternalFileEUPercodeVldKey_ as return 0')
end
go

print 'Altering stored procedure ExternalFileEUPercodeVldKey_'
go

alter procedure dbo.ExternalFileEUPercodeVldKey_ (
    @pExternalFileID int,
...
Description:
    Checks deletes potential PK violations in the EU percode 
...
-- Declare Cursor
---------------------------------------------------------------------
insert into @Percode (ExternalFileID, Percode, DuplicateCount)
select ExternalFileID, Percode, count(*)
  from dbo.ExternalFileEUPercodeStaging
 where ExternalFileID = @pExternalFileID
 group by ExternalFileID, Percode
having count(*) > 1

declare Key_Cursor cursor local dynamic for
select a.ExternalFileID, a.Percode
  from dbo.ExternalFileEUPercodeStaging a
       inner join
       @Percode b on a.ExternalFileID = b.ExternalFileID and 
                     a.Percode = b.Percode
for update
...
while( 1=1 ) begin
    ...
    fetch next from Key_Cursor into @ExternalFileID, @PercodeID
    select @Error = @@error
    if( @Error <> 0 ) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = isnull(convert(varchar,@Error),'NULL')
    ...
    if exists (select *
                ...
                where ExternalFileID = @ExternalFileID and
                      Percode = @PercodeID and
                      DuplicateCount > 1)
        begin
            delete from dbo.ExternalFileEUPercodeStaging
