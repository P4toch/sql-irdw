if object_id('dbo.ExternalFileEUTravelerCategoryVldKey_') is null begin
    execute('create procedure dbo.ExternalFileEUTravelerCategoryVldKey_ as return 0')
end
go

print 'Altering stored procedure ExternalFileEUTravelerCategoryVldKey_'
go

alter procedure dbo.ExternalFileEUTravelerCategoryVldKey_ (
    @pExternalFileID int,
...
Description:
    Checks deletes potential PK violations in the EU traveler category 
...
declare     -- SP specific constants and variables
    ...
    @ExternalFileID                 int, 
    @Comcode                        int,
    @TravelerCategoryID             int,
    @DuplicateCount                 int

declare @TravelerCategory table
    (ExternalFileID int,
     Comcode int,
     TravelerCategoryID int,
     DuplicateCount int)
...
-- Declare Cursor
---------------------------------------------------------------------
insert into @TravelerCategory (ExternalFileID, ComCode, TravelerCategoryID, DuplicateCount)
select ExternalFileID, Comcode, TravelerCategoryID, count(*)
  from dbo.ExternalFileEUTravelerCategoryStaging
 where ExternalFileID = @pExternalFileID
 group by ExternalFileID, Comcode, TravelerCategoryID
having count(*) > 1

declare Key_Cursor cursor local dynamic for
select a.ExternalFileID, a.Comcode, a.TravelerCategoryID
  from dbo.ExternalFileEUTravelerCategoryStaging a
       inner join
       @TravelerCategory b on a.ExternalFileID = b.ExternalFileID and 
                     a.Comcode = b.Comcode and
                     a.TravelerCategoryID = b.TravelerCategoryID
for update
...
while( 1=1 ) begin
    ...
    fetch next from Key_Cursor into @ExternalFileID, @Comcode, @TravelerCategoryID
    select @Error = @@error
    if( @Error <> 0 ) begin
        select @ErrorCode   = @ERRUNEXPECTED,
               @MsgParm1    = isnull(convert(varchar,@Error),'NULL')
    ...
    if exists (select *
                 from @TravelerCategory
                where ExternalFileID = @ExternalFileID and
                      Comcode = @Comcode and
                      TravelerCategoryID = @TravelerCategoryID and
                      DuplicateCount > 1)
        begin
            delete from dbo.ExternalFileEUTravelerCategoryStaging
            ...
            update @TravelerCategory
               set DuplicateCount = DuplicateCount - 1
             where ExternalFileID = @ExternalFileID and
                   Comcode = @Comcode and
                   TravelerCategoryID = @TravelerCategoryID
